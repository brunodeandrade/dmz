...
static unsigned int packet_processing(u_int16_t thread_id,
  ...
  print_info(flow->upper_name, flow-> bytes, flow->protocol,flow->lower_port,flow->packets);
  add_to_hash(flow->upper_name, flow-> bytes, flow->protocol,(int)flow->lower_port,flow->packets);
